...
export default function PdfToJpgPage() {
  ...
  const handleConvert = async () => {
    ...
    try {
      const response = await axios.post(`${API_URL}/api/convert/pdf-to-jpg`, formData, {
        responseType: 'blob',
        onUploadProgress: (progressEvent) => {
          const percentCompleted = Math.round((progressEvent.loaded * 100) / (progressEvent.total || 1));
          setUploadProgress(percentCompleted);
          if (percentCompleted === 100) setStatus('Converting to images...');
        },
      });
  ...
  const handleDelete = async () => {
    if (!zipUrl) return;
    try {
      await axios.post(`${API_URL}/api/cleanup`, { files: ['images.zip'] });
      reset();
    } catch (error) {
    ...
    <div className="bg-gray-50 dark:bg-black min-h-[calc(100vh-64px)] py-12 transition-colors">
      <div className="container mx-auto px-4">
        <div className="text-center mb-12">
          <div className="inline-flex h-16 w-16 items-center justify-center rounded-2xl bg-yellow-500 text-white mb-4 shadow-lg">
            <ImageIcon className="h-8 w-8" />
          </div>
          <h1 className="text-4xl font-extrabold text-black dark:text-white mb-2">PDF to JPG</h1>
          <p className="text-gray-500 text-lg">Extract all images that are within a PDF or convert each page to a JPG image.</p>
        </div>
        ...
        {!zipUrl ? (
          <div className="flex flex-col items-center">
            ...
            {files.length > 0 && !isProcessing && (
              <div className="mt-12">
                <Button 
                  size="lg" 
                  className="h-16 px-12 text-xl font-bold rounded-xl shadow-xl hover:scale-105 transition-transform bg-yellow-500 hover:bg-yellow-600"
          ...
          <div className="max-w-md mx-auto bg-white dark:bg-gray-900 rounded-3xl p-12 text-center shadow-2xl border border-gray-100 dark:border-gray-800">
            ...
            <h2 className="text-2xl font-bold text-black dark:text-white mb-2">PDF converted to JPG!</h2>
            <p className="text-gray-500 mb-8">All pages have been converted to JPG and are ready for download.</p>
            
            <div className="flex flex-col gap-4">
              <a 
                href={zipUrl} 
                download="images.zip"
                className="flex items-center justify-center gap-2 w-full bg-yellow-500 text-white py-4 px-8 rounded-xl font-bold text-lg hover:bg-yellow-600 transition-colors shadow-lg"
              >
